<div class="lg-pager" :class="{'lg-pager-left':position=='left','lg-pager-center':position=='center'}">	
	<ul> 
		<li class="lg-pager-item" v-if="pager.page>10" :class="{'disable':pager.cur==1}">
		...
		<li class="lg-pager-item" v-if="pager.page>10" :class="{'disable':pager.cur==pager.page}">
			<a href="javascript:" @click="to(pager.page,pager.cur==pager.page)">尾页</a>
		</li>
		<li class="lg-pager-shortcut">去第 <input type="text" v-model="shortcut"> 页<a href="javascript:" class="lg-pager-shortcut-confirm" @click="to(shortcut)" >确定</a>每页<select v-model="pager.size" @change="select(pager.size)"><option v-for="o in pager.option" :value="o" v-html="o+'条'"></option></select>共{{pager.page}}页，{{pager.total}}条 
		</li>
	</ul>
</div>
